...
let previousCollisions = new Set(); // To track old collision pairs
...
async function initLiveApp() {
  ...
  // Przycisk czyszczący
  document
    .getElementById("clearSelectedShips")
  ...
  shipsArray.forEach((ship) => {
    ...
    if (!hasLength) {
      ...
      if (!marker) {
        ...
        marker = L.marker([latitude, longitude], { icon }).on("click", () =>
          selectShip(mmsi)
        );
...
function updateCollisionsList() {
  ...
  finalColls.forEach((c) => {
    ...
    if (!previousCollisions.has(key)) {
      newCollision = c; // Mark as a new collision
    }
    // splitted circle
    ...
    const mark = L.marker([latC, lonC], { icon: collisionIcon })
      .bindTooltip(`Collision: ${shipAName} & ${shipBName}`, {
        direction: "top",
        sticky: true,
      })
    ...
    // Highlight new collision
    if (newCollision === c) {
      mark.setOpacity(0.7);
      setTimeout(() => mark.setOpacity(1), 500);
    }

    // Zoom button
    item.querySelector(".zoom-button").addEventListener("click", () => {
      zoomToCollision(c);
      updateVectorSlider(c.tcpa);
    });
...
function updateVectorSlider(tcpa) {
  let newVectorLen = Math.round(tcpa);
  newVectorLen = Math.max(newVectorLen, 1); // Minimum 1 minute
...
function updateSelectedShipsInfo(forceRedraw) {
  ...
  function getShipData(mmsi) {
    ...
    if (shipPolygonLayers[mmsi]?.shipData)

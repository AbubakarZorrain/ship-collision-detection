...
function createShipIcon(shipData, isSelected, mapZoom = 5) {
  ...
  const hdg = shipData.heading != null ? parseFloat(shipData.heading) : parseFloat(shipData.cog || 0);

  let highlightRect = isSelected
  ...
  if (mapZoom >= 14 && dimA !== null && dimB !== null) {
    shipShape = `
      <!-- Realistic Ship Body -->
      <polygon points="0,-12 8,-6 8,6 6,10 -6,10 -8,6 -8,-6"
               fill="${color}" stroke="black" stroke-width="1.5" />
      
      <!-- Ship Bridge -->
      <rect x="-4" y="-8" width="8" height="5" fill="white" stroke="black" stroke-width="1" />
      
      <!-- Windows -->
      <circle cx="-2" cy="-6" r="0.8" fill="black" />
      <circle cx="0" cy="-6" r="0.8" fill="black" />
      <circle cx="2" cy="-6" r="0.8" fill="black" />
  ...
  // ** Ensure correct scaling and rotation **
  const svgSize = 32;
...
function createShipPolygon(shipData) {
  console.log("Creating polygon for:", shipData.mmsi)

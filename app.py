...
def calculate_cpa_tcpa_2(ship_pair):
    print(f"Calculating CPA/TCPA for pair: {ship_pair}")
    ship_a, ship_b, distance, approach_angle = ship_pair

    # Extract velocity components
    vx1, vy1 = math.cos(math.radians(get_heading_or_cog(ship_a))) * ship_a["sog"], math.sin(math.radians(get_heading_or_cog(ship_a))) * ship_a["sog"]
    vx2, vy2 = math.cos(math.radians(get_heading_or_cog(ship_b))) * ship_b["sog"], math.sin(math.radians(get_heading_or_cog(ship_b))) * ship_b["sog"]
    ...
    rx = ship_b["latitude"] - ship_a["latitude"]
    ry = ship_b["longitude"] - ship_a["longitude"]

    # Convert to nautical miles
    NM_PER_DEGREE_LAT = 60
    NM_PER_DEGREE_LON = 60 * math.cos(math.radians(ship_a["latitude"]))  # Adjust for latitude

    rx_nm = rx * NM_PER_DEGREE_LAT
    ry_nm = ry * NM_PER_DEGREE_LON

    # Time to CPA (TCPA)
    vr_dot_r = vrx * rx_nm + vry * ry_nm
    vr_mag_squared = vrx**2 + vry**2
    if vr_mag_squared == 0:
        tcpa = float('inf')  # Ships are stationary relative to each other
    else:
        tcpa = (-vr_dot_r / vr_mag_squared) * 60  # Convert to minutes

    # Closest Point of Approach (CPA) in Nautical Miles
    cpa_nm = math.sqrt(rx_nm**2 + ry_nm**2 + 2 * vr_dot_r * tcpa + vr_mag_squared * tcpa**2)

    return (ship_a, ship_b, cpa_nm, tcpa, distance)
...
def collisions():
    ...
    WITH recent_collisions AS (
      ...
      FROM `ais_dataset_us.ships_collisions` c
    ...
    print(f"Returning {len(out)} collisions.")
    return jsonify(out)
